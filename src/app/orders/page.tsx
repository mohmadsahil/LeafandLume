        ...
        <ul className="mt-8 space-y-4">
          {orders.map((o) => {
            ...
            return (
              <li
                key={o.id}
                className="shadow-soft hover:shadow-luxury rounded-3xl border bg-card p-4 transition-shadow sm:p-6"
              >
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <p className="font-mono text-xs text-muted-foreground">#{o.id}</p>
                    <h2 className="mt-1 break-words font-heading text-base font-semibold sm:text-lg">
                      {o.items.join(' · ')}
                    </h2>
